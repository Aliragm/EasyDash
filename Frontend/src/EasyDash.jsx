...
const EasyDash = ({ token, onLogout }) => {
  ...
  const [tempId, setTempId] = useState('sensor_sala_01'); 
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 p-6 font-sans">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {/* HEADER & CONFIG (AGORA FICA SEMPRE VISÍVEL) */}
        ...
        {/* LÓGICA DE EXIBIÇÃO: Se der erro, mostra só o erro AQUI EMBAIXO, não na tela toda */}
        
        {error ? (
          <div className="bg-red-500/10 border border-red-500 text-red-500 p-10 rounded-xl text-center">
            <h2 className="text-xl font-bold mb-2">Erro de Acesso</h2>
            <p>{error}</p>
            <p className="text-sm text-slate-400 mt-4">Tente digitar outro ID de sensor acima.</p>
          </div>
        ) : (
          <>
            {loading && history.length === 0 ? (
               <div className="text-white p-10 text-center">Carregando dados do IoT...</div>
            ) : (
               <>
                {/* CARDS DE MÉTRICAS */}
